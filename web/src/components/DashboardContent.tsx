...
import type { DashboardData, ReportListItem, Severity } from '@/types';
...
export default function DashboardContent({ dashboard }: { dashboard: DashboardData }) {
  const { t } = useLocale();
  ...
  const sevKeys = ['critical', 'major', 'minor', 'info'] as Severity[];
  const dimKeys = ['efficiency', 'security', 'cost', 'errors'] as const;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        />
        <StatCard
          label={t('dash.total_findings')}
          value={totalFindings}
          color="text-accent-yellow"
          subLabel={`${sevDist.critical ?? 0} ${t('dash.critical')}`}
          icon={
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card space-y-4">
          <h2 className="font-semibold text-slate-200">{t('dash.severity_dist')}</h2>
        ...
        <div className="card space-y-4">
          <h2 className="font-semibold text-slate-200">{t('dash.dimension_dist')}</h2>
          <div className="space-y-3">
            {dimKeys.map((dim) => (
              <BarRow
                key={dim}
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-slate-200">{t('dash.recent_reports')}</h2>
          <Link href="/reports" className="text-sm text-accent-blue hover:underline">
            {t('dash.view_all')}
          </Link>
        </div>
            ...
            <table className="w-full text-sm" aria-label="Recent reports">
              <thead>
                <tr className="border-b border-surface-border">
                  <th scope="col" className="pb-3 text-left font-semibold text-slate-400">{t('table.repository')}</th>
                  <th scope="col" className="pb-3 text-left font-semibold text-slate-400 hidden sm:table-cell">{t('table.date')}</th>
                  <th scope="col" className="pb-3 text-left font-semibold text-slate-400">{t('table.status')}</th>
                  <th scope="col" className="pb-3 text-right font-semibold text-slate-400">{t('table.findings')}</th>
                  <th scope="col" className="pb-3 text-right font-semibold text-slate-400 hidden md:table-cell">{t('table.duration')}</th>
                </tr>
              </thead>
              <tbody>
                {recent_reports.slice(0, 5).map((r: ReportListItem) => (
                  <tr key={r.id} className="border-b border-surface-border last:border-0 table-row-hover">
